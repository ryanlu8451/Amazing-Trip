...
第一階段已開始 Firestore sharing，但仍屬 MVP / private testing model，不是完整 invite system。
...
- Sharing invite 尚未有 accepted / pending status。
...
- Flights / Hotels / Budget 的長表單仍偏複雜，建議下一輪繼續做 Basic / Details 分段或 progressive disclosure。
