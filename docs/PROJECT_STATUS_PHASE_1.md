...
最近一次驗證時間：UI/UX 第一波簡化後已重新執行 `npm run lint` 與 `npm run build`，兩者皆通過。
...
- 朋友從 Safari / Chrome 登入 shared trip
- Owner 建立 trip 後分享 Gmail 給 friend
- Friend 登入後是否看到 shared trip
- Viewer 是否真的不能編輯：
  - Timeline
  - Flights
  - Hotels
...
1. 完成並 commit UI/UX 第一波簡化與文件更新。
2. Deploy 最新版本到 Firebase Hosting。
3. Deploy Firestore rules。
4. 用正式網址完成一輪 owner/editor/viewer 測試。
5. 測試手機 Safari / Chrome login。
6. 測試 PWA install icon。
7. 繼續做 Flights / Hotels / Budget 長表單簡化。
8. 再決定是否進入 Phase 2 sharing UX。
