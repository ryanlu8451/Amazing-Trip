...
產品內 install guidance 已完成：

- `src/components/InstallGuide.jsx`：共用 mobile install guide。
- `src/pages/Login.jsx`：未登入前顯示 app-store-like install instructions。
- `src/pages/Tips.jsx`：Settings 中提供可重看的 install guide。
- `src/lib/i18n.js`：英文 / 繁中安裝指引文案。

指引內容涵蓋：

- 使用正式 Firebase Hosting URL。
- iPhone Safari：Share → Add to Home Screen。
- Android Chrome：Install app / Add to Home screen。
- Google 登入避免 LINE / Instagram / Facebook 等 app 內建瀏覽器。
- 本機測試使用 `localhost`，除非 `127.0.0.1` 已加入 Firebase Auth authorized domains。
...
目前 build 通過，並已完成 route-level code splitting。
...
- Lint 通過
- Build 通過
- Vite 500 kB chunk size warning 已消除
...
- `src/App.jsx` 使用 `lazy()` / `Suspense` 拆分 route page。
- Home / Timeline / Flights / Hotels / Budget / Settings / Invite 等頁面已獨立 chunk。
...
- `src/pages/Login.jsx`
  ...
  - 保留 Google-only CTA
  - 新增未登入前語言切換
  - 新增未登入前 mobile install guide
- `src/pages/Home.jsx`
  - Create / Edit Trip 移除 members input
  - Travel Members card 依 `tripType` 顯示 solo 或 group invite 狀態
  - 新增 first-run onboarding flow 入口
- `src/components/OnboardingModal.jsx`
  - 建立旅程、匯入訂單、邀請成員、手機安裝四步驟導覽
- `src/components/InstallGuide.jsx`
  - Login / Settings 共用的 PWA 安裝指引
- `src/components/FormSection.jsx`
  - 共用可折疊表單區塊
  - 用於降低 Flights / Hotels / Budget 長表單的視覺負擔
  - 支援 controlled open state，供 import auto-fill 後自動展開 optional details
- `src/pages/Flights.jsx`
  - 航班表單分成必要資訊與選填細節
  - 費用、航廈、登機門、座位、訂位代號、狀態、備註收進 optional details
  - 匯入文字 / PDF 成功後自動展開 optional details
  - Save button 改為 sticky footer
- `src/pages/Hotels.jsx`
  - 住宿表單分成必要資訊、住宿與費用、選填細節
  - WiFi、電話、地址、地圖、訂房編號、備註收進 optional details
  - 匯入文字 / PDF 成功後自動展開 optional details
  - Save button 改為 sticky footer
- `src/pages/Budget.jsx`
  - 預算分配與實際花費表單分成必要資訊與選填細節
  - 自訂名稱、emoji、備註收進 optional details
  - Set budget / allocation / expense / imported spending save actions 改為 sticky footer
- `src/pages/Tips.jsx`
  - Settings 可重新開啟產品導覽
  - Settings 可重新查看 mobile install guide
- `src/store/settingsStore.js`
  - 新增 `hasSeenOnboarding`
  - 新增 `completeOnboarding()` / `resetOnboarding()`
...
### 12.6 App-like Release Readiness

本專案不實際上架 App Store / Google Play，但技術收尾目標是接近 app-ready PWA。

1.0.0 已訂為 QA Test Release。版本功能、驗證指令與後續版本編輯規則請維護在 `docs/RELEASE_NOTES.md`。

已完成：

- Firebase Hosting production build。
- PWA manifest / icon / service worker。
- Mobile install guide。
- Route-level code splitting。
- Firestore rules compile dry-run audit。
...
- Android Chrome Install app。
- Installed PWA Google login。
- 多帳號 owner/editor/viewer 權限回歸。
- Firebase Auth authorized domains。
- Privacy / terms / support documents。

### 12.7 Booking Import 技術方向

Flights / Hotels 訂單文字與 PDF 導入已完成第一版。

目前採本機解析，不使用付費 OCR / AI API：
